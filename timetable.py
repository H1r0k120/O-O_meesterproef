...
acc_token = "8jcm5uhbpn9nogsl7kcickuaq1"  # Replace with a valid token
...
for appointment in appointments["response"]["data"]:
    ...
    schedule_data.append([date, start_time, end_time, subject, teacher, location])

# Convert to DataFrame
df = pd.DataFrame(schedule_data, columns=["Date", "Start Time", "End Time", "Subject", "Teacher", "Location"])

df["Sort Time"] = pd.to_datetime(df["Date"] + " " + df["Start Time"])  # Combine Date & Start Time
